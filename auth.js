...
class AuthManager {
    ...
    showMainApp() {
        ...
        // Инициализация мультиплеерного клиента с аутентификацией
        this.initializeMultiplayerClient();
    }
    
    initializeMultiplayerClient() {
        console.log('🔌 Инициализация мультиплеерного клиента...');
        
        if (typeof MultiplayerClient !== 'undefined') {
            // Создаем глобальный экземпляр мультиплеерного клиента
            window.multiplayerClient = new MultiplayerClient();
            console.log('✅ Мультиплеерный клиент инициализирован');
        } else {
            console.log('⚠️ MultiplayerClient не найден, мультиплеер недоступен');
        }
    }
